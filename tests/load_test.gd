...
func _check_dzien50_data() -> int:
	...
	failures += _check_localization()

	return failures


## Every key in the translation CSV must have a non-empty English column, and
## the compiled catalog must exist (regenerated by --import from strings.csv).
func _check_localization() -> int:
	var failures := 0
	if not FileAccess.file_exists("res://localization/strings.en.translation"):
		push_error("missing compiled translation res://localization/strings.en.translation")
		failures += 1
	var file := FileAccess.open("res://localization/strings.csv", FileAccess.READ)
	if file == null:
		push_error("missing res://localization/strings.csv")
		return failures + 1
	...
	while not file.eof_reached():
		...
		if row.size() < 1 or row[0] == "":
			continue
		keys += 1
		if row.size() < 2 or row[1].strip_edges() == "":
			push_error("untranslated key: %s" % row[0].left(60))
			missing += 1
	file.close()
	print("localization: %d keys, %d untranslated" % [keys, missing])
